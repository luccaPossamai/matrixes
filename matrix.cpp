...
std::filesystem::path matrixes() { return COMPILE_PATH / "matrixes"; }
...
int main(int argc, char *argv[]) {
  ...
  if (std::filesystem::exists(matrixes()) && !matrixMenu)
    menu = new ListMenu(matrixes());
  float acc = 100;
...
void readKeys() {
  ...
  if (matrixMenu) {
    matrixMenu->selectedFrame = Clamp(matrixMenu->selectedFrame + add, 0,
                                      matrixMenu->frames.size() - 1);
    if (IsKeyPressed(KEY_SPACE) || IsKeyPressedRepeat(KEY_SPACE)) {
      matrixMenu->selectedFrames.at(matrixMenu->selectedFrame) =
          !matrixMenu->selectedFrames.at(matrixMenu->selectedFrame);
      matrixMenu->selectedFrame = Clamp(matrixMenu->selectedFrame + 1, 0,
                                        matrixMenu->frames.size() - 1);
    }
  } else if (menu) {
    menu->selected = Clamp(menu->selected + add, 0, menu->arcs.size() - 1);
    int firstAdd =
        menu->selected - menu->first < menu->off && add < 0 ? -1
        : menu->first + menu->number - 1 - menu->selected < menu->off && add > 0
            ? +1
            : 0;
    menu->first =
        Clamp(menu->first + firstAdd, 0, menu->arcs.size() - menu->number);
